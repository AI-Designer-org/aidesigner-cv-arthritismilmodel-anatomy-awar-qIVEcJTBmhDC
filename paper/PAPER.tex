...
Detection-based MIL---where a detector first localises instances before feature extraction---offers advantages over tile-sampling MIL when instance-level annotations are available. For arthritis, this approach naturally handles variable joint counts, occlusions, and the clinical reality that different patients have different numbers of visible joints~\cite{scirep2025}. For multi-view scenarios, cross-fusion architectures such as XFMamba~\cite{carion2025} show that combining anatomical perspectives can improve classification in general MSK abnormality detection, motivating our multi-view fusion design. Our architecture extends detection-based MIL to the multi-disease setting with multi-view support, which to our knowledge has not been attempted.

\subsection{Foundation Models for Musculoskeletal Radiography}
Self-supervised foundation models pretrained on large-scale medical imaging data offer a path to label-efficient transfer learning. DINOv2~\cite{dinov22023} provides strong off-the-shelf visual features from ImageNet-scale self-supervision. More domain-specific models have recently been described in preprint reports: OrthoFoundation~\cite{orthofoundation2026} reports training on 1.2 million knee X-ray and MRI images using DINOv3, achieving strong performance on 14 downstream tasks with 50\% label efficiency on knee pathology benchmarks. SKELEX~\cite{skelex2026} extends this to 1.2 million general MSK radiographs, demonstrating cross-anatomy transfer to fracture detection and OA grading in its reported experiments.
...
The test suite runs on CPU with the \texttt{tiny\_debug} backbone; when a CUDA-capable GPU is available, the test code auto-selects it for gradient-based and mixed-precision tests. Runtime for the full 39-test suite is under 2 minutes on CPU with \texttt{tiny\_debug}. The profiling estimates for DINOv2 ViT-L/14 (Section~\ref{sec:appendix-profiling}) are architect-derived calculations based on operator FLOPs and parameter counts, not measured end-to-end timings with real data.
...
\bibitem{autoscora2026}
  autoscoRA: Deep learning to automate Sharp/van der Heijde scoring of rheumatoid arthritis from hand and foot radiographs (2026).
...
\bibitem{psavit2024}
  Vision transformer model for automated radiographic assessment of joint damage in psoriatic arthritis (2024).
  In {\it MLMI 2024 (MICCAI Workshop)}.

\bibitem{scirep2025}
  Multistage deep learning for Sharp score prediction in rheumatoid arthritis (2025).
...
\bibitem{orthofoundation2026}
  OrthoFoundation: A multimodal vision foundation model for generalizable knee pathology (2026).
  {\it arXiv preprint arXiv:2601.18250}.

\bibitem{skelex2026}
  SKELEX: A generalizable large-scale foundation model for musculoskeletal radiographs (2026).
  {\it arXiv preprint arXiv:2602.03076}.
...
\bibitem{carion2025}
  XFMamba: Cross-fusion Mamba for multi-view medical image classification (2025).
  In {\it Proceedings of MICCAI 2025}.
...
The test suite was executed on CPU using the \texttt{tiny\_debug} backbone (2 conv layers + linear projection to 64-dim) with PyTorch 2.3.0 and Python 3.12. When a CUDA-capable GPU is available, the test harness auto-selects it for bf16/fp16 numerical stability and gradient-flow tests. Full repro command:
...
\label{sec:appendix-profiling}
The profiler script (\texttt{profile\_model.py}) provides architect-derived estimates for the following per-batch costs for the full DINOv2 ViT-L/14 model — the reference deployment configuration — on a single NVIDIA RTX 4090 (B=16, $N_{\max}$=30). These figures are calculated from parameter counts and operator FLOPs rather than measured end-to-end timings with real data:
...
\textbf{Yes.} The reference implementation includes a complete PyTorch codebase with a typed \texttt{ModelConfig}, 7 pipeline-stage modules, a 39-test validation suite, an ablation runner (7 ablation configurations: 6 fully runnable, 1 with partial implementation as a config-only path for tile sampling), and a profiling script. Image-level ViT and single-disease specialist baselines are defined as ablation configurations but are not yet fully separated as standalone modules. All code is released under an open-source license.
