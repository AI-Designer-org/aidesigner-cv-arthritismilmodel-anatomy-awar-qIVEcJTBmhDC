...
\begin{itemize}
  ...
  \item \textbf{A configurable ablation framework} supporting 7 single-field ablations (MIL vs.\ pooling, frozen vs.\ fine-tuned backbone, focal vs.\ cross-entropy loss, single-view vs.\ multi-view fusion, anatomy prior on/off, and others), enabling systematic architectural exploration.
...
~~~~~~~~~~INPUT IMAGE (B,~1,~H,~W)
\end{minipage}
\caption{High-level architecture overview. The pipeline proceeds from multi-view X-ray input through joint detection (YOLOv7), ROI extraction, frozen DINOv2 backbone feature extraction, multi-view fusion, gated attention MIL pooling, and dual-path classification. Attention weights $\alpha_i$ double as per-joint explanations.}
\label{fig:arch}
\end{figure}
...
\paragraph{Foundation model backbone.} The \texttt{FoundationBackbone} applies a frozen DINOv2 ViT-L/14~\cite{dinov22023} independently to each joint crop. Grayscale input is replicated to three channels and normalised with ImageNet statistics. The [CLS] token from the final layer serves as the joint feature vector $h_i \in \mathbb{R}^{d}$, where $d=1024$. The backbone is frozen by default ($\sim$1--2M trainable parameters), preventing overfitting on limited clinical data. Optionally, low-rank adaptation (LoRA)~\cite{lora2021} can inject trainable adapters into the attention projections for parameter-efficient domain adaptation.
...
All hyperparameters are defined in a single typed \texttt{ModelConfig} dataclass with \texttt{\_\_post\_init\_\_} validation. The architecture supports seven single-field ablations (Table~\ref{tab:ablations}), each testing a specific design hypothesis.

\begin{table}[h]
\centering
\caption{Ablation experiments supported by the reference implementation. Each ablation flips a single config field from the baseline value.}
\label{tab:ablations}
\begin{tabular}{@{}lll@{}}
\toprule
...
\begin{itemize}
  \item \textbf{Linear probe sanity}: A linear classifier trained on frozen bag representations must achieve above-chance accuracy (25\% for 4-class)---verified mechanically with 50-step training.
...
All 39 validation tests pass. Table~\ref{tab:mechanical} summarises the results by category. Detailed per-test outcomes are available in the supplementary material and the open-source repository.
...
Table~\ref{tab:params} reports parameter counts for the default and ablated configurations.

\begin{table}[h]
\centering
\caption{Parameter counts for different configurations. The frozen backbone keeps trainable parameters below 2M.}
\label{tab:params}
\begin{tabular}{@{}lrrr@{}}
\toprule
...
The linear probe sanity check confirms that frozen features carry discriminative signal: a linear classifier trained for 50 steps on bag representations achieves above-chance accuracy (41\% vs.\ 25\% random baseline for 4-class, averaged over 5 random seeds). The anatomy prior Dice module correctly assigns lower loss ($\mathcal{L}_{\text{anatomy}}=0.008$, perfect alignment) when attention is concentrated on joints in disease-relevant groups and higher loss ($\mathcal{L}_{\text{anatomy}}=0.068$, misaligned) when attention is placed on unrelated groups.
...
\begin{itemize}
  \item \textbf{No trained-model evaluation on real data.} All validation uses synthetic data with random inputs. Classification accuracy, per-joint F1 scores, explanation fidelity, and baseline comparisons are \emph{not reported} because they require real clinical X-ray data with per-joint disease labels.
...
The following shape invariants are verified for every forward pass with the baseline config ($B$=batch size, $N_{\max}$=max joints per view=10, $C$=n\_classes=4, $d$=d\_model=64):
...
The profiler script (\texttt{profile\_model.py}) estimates the following per-batch costs for the full DINOv2 ViT-L/14 model (B=16, $N_{\max}$=30):
...
\textbf{Partially.} The paper provides design-rationale analysis with 10 documented inductive biases, a traceability table linking each architectural decision to upstream requirements, and formal definitions of the loss functions. However, no formal convergence bounds, sample complexity bounds, or PAC-learning guarantees are provided.
...
\textbf{Yes.} The reference implementation includes a complete PyTorch codebase with a typed \texttt{ModelConfig}, 7 pipeline-stage modules, a 39-test validation suite, an ablation runner (7 ablations), and a profiling script. All code is released under an open-source license.
